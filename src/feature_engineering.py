...
class Strategy(ABC):
    @abstractmethod
    def ApplyTransformation(self, s:pd.Series ,df: pd.DataFrame) -> None:
        """Abstract method to execute a strategy on the DataFrame."""
        pass
class MapBinarySeries(Strategy):
    def ApplyTransformation(self,s: pd.Series) -> pd.Series:
        """
    ...
    a       re deterministic and must be consistent between training and serving.
    ...
    # Return unchanged - will be handled by one-hot encoding
        return s
class BuildFeatures(Strategy):
    def ApplyTransformation(self ,df: pd.DataFrame, target_col: str = "Churn") -> pd.DataFrame:
        """
    ...
    # Convert 2-category features to 0/1 using deterministic mappings
        for c in binary_cols:
            ...
            k=MapBinarySeries()
            df[c] = k.ApplyTransformation(df[c].astype(str))
